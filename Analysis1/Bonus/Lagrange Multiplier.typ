...
- Standard Linear Algebra theory
- Differential calculus
- The general concept of manifolds embedded in $bb(R)^n$

= The problem

Our motivation in studying this specific problem is to have an easy solution for all problems of the following form:

1) we have a manifold M defined as the pre-image of a regular value of a function g, i.e. $M=g^(-1)(0)$, with $0$ (not the scalar, the null vector in $R^p$) being a regular value of $g:bb(R)^n->bb(R)^p$ (dimension of $M$ will be $d=n-p$),

REMINDER: A value $c$ is called a _regular value_ of a function $g$ if $g$ is a _submersion_ ($D g$ is surjective) at every point of $g^(-1)(c)={x in bb(R)^n|g(x)=c}$

2) we have a function $F:bb(R)^n -> bb(R)$ that is defined on the WHOLE ambient space $bb(R)^n$ and we now want to study the extrema of the function $f=F|M$, i.e. $F$ with its domain restricted to the manifold $M$, but
...
(Note: for the sake of clarity, I will be assuming that all functions are well behaved and derivatives well defined wherever I use them, without explicitly stating it)
...
$D f (x_0) v$ essentially means 'the rate of change of the value of $f$ as we move along direction v'. In the context of $M$, we cannot move along any direction; we can only move along directions in its tangent space $T_x_0 M$!
...
The last key we need to remember is a property we learned in Linear Algebra last semester, that if $v in "span"{u_i}$ then "$v$ can be written as a linear combination of the vectors $u_i$", or "there exists a vector $lambda$ such that $v = u lambda = sum u_i lambda_i$". If we write take $u_i = nabla g_i$, then $N_x_0 M = "span"{nabla g_i (x_0)}$, so $v in N_x_0 M <=> v in "span"{nabla g_i (x_0)} <=> exists lambda in bb(R)^p ,v = sum_(i=1)^p nabla g_i lambda_i = (D g (x_0))^T lambda$.

REMINDER: For any function $f: bb(R)^n -> bb(R)$, $nabla f = (D f)^T$

So our final rewritten version would be:
$ nabla F (x_0) = sum_(i=1)^p (nabla g_i (x_0))lambda_i $
#align(center,[
and by transposing both sides, we transform gradients to total derivatives, $nabla -> D$, we get:
])
$ (nabla F (x_0))^T = (sum_(i=1)^p (nabla g_i (x_0))lambda_i)^T $
$ D F (x_0) = sum_(i=1)^p lambda_i (nabla g_i (x_0))^T $
$ D F (x_0) = sum_(i=1)^p lambda_i (D g_i (x_0)) $
...
3) That however just means that the total derivative is perpendicular to the tangent space, so it is part of the _normal_ space $D F (x_0) bot v forall v in T_x_0 M => D F (x_0) bot T_x_0 M => D F (x_0) in N_x_0 M$.
...
5) By transforming $nabla g_i$ into $D g_i$ (transpose) we are now dealing with row vectors instead of column vectors, so we can now replace the gradients with derivatives and say $D F (x_0)$ is a linear combination of the row vectors $D g_i (x_0)$.

6) We can express that linear combination as "existence of a linear functional, a.k.a. transposed vector", $exists lambda in bb(R)^p$ such that $D F (x_0) = lambda^T D g (x_0)$. This particular value of $lambda$, which we can solve for algebraically, is known as the "Lagrange Multiplier".
...
= Symbols Used
- $g^(-1)(x_0)$: pre-image of $x_0$ under $g$
- $x_0$: a specific point of the manifold in question
- $D f$: total derivative of a function $f$, equal to its Jacobian
- $F$: the target function $F:bb(R)^n->bb(R)$
- $f$: the target function with its domain restricted to the manifold ($f=F|_M$)
- $g$: the function that defines the manifold
- $n$: the number of dimensions of the ambient space $bb(R)^n$
- $p$: the number of dimensions of the output of $g$ (the dimensions of the resulting manifold are $n-p$)
- $lambda$: the Lagrange Multiplier, a particular value that represents the linear combination
